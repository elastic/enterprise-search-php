...
product="elastic/enterprise-search-php"
...
if [[ "$CMD" == "assemble" ]]; then
    artefact_name="enterprise-search-php-${VERSION}"
    echo -e "\033[34;1mINFO: copy artefacts\033[0m"
    rsync -arv --exclude=.github --exclude=.git "$PWD" "${output_folder}/."

    echo -e "\033[34;1mINFO: rename artefacts\033[0m"
    mv -v "${output_folder}/enterprise-search-php" "${output_folder}/${artefact_name}"

    echo -e "\033[34;1mINFO: build artefacts\033[0m"
    cd ./.github/output && tar -czvf ${artefact_name}.tar.gz "${artefact_name}/." && cd -
